...
\end{equation}
A questo punto abbiamo identificato due situazioni: $\textit{x}<\mathit{X_{max}}$ e $\textit{x}\geq \mathit{X_{max}}$. Unendo le Equazioni \ref{eq:DF_no_asint} e \ref{eq:df_asintoto} otteniamo l'\MyEq{eq:DF_totale}, equazione finale del Dynamic Fanout, rappresentato in \myFig{fig:DF_tot_no_arr}.
\begin{equation}
\scriptsize
\label{eq:DF_totale}
